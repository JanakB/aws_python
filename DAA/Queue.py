...
IAtime = [0,4,1,5,2,3,4,1,2,3]
service_time = [1,2,3,4,2,3,1,2,1,3]
...
for i in range(len(customers)):
    print(f'{customers[i]} \t\t {IAtime[i]} \t\t\t\t {service_time[i]} \t\t {service_begin[i]} \t\t{service_end[i]}')
print("\n")

print(f"Average waiting Time in the system: {W}")
print(f"Average Waiting in the queue: {Wq}")
print(f"Time Average: {L}")
print(f"Time Average in Queue: {Lq}")
print(f"Server Busy Time: {server_busy_time}")